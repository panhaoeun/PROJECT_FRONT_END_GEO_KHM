                ...
                >
                    ...
                    <div class="thumb-list">
                        <img
                            ...
                            :class="{
                                'choosed-thumb': thumb.id === choosedThumb.id,
                            }"
                        />
                    </div>
...
export default {
    ...
    mounted() {
        this.runImage();
        window.addEventListener("resize", this.runImage);
    },
    beforeUnmount() {
        window.removeEventListener("resize", this.runImage);
    },
    watch: {
        choosedThumb(thumb) {
            const matchNormalImg = this.normal_size.find(
                (img) => img.id === thumb.id
            );
            const matchLargeImg = this.large_size.find(
                (img) => img.id === thumb.id
            );
    ...
    created() {
        console.log(this.baseImages.normal_size)
        if (Object.keys(this.baseImages).length > 0) {
            for (const key in this.baseImages) {
                if (
                    Object.prototype.hasOwnProperty.call(this.baseImages, key)
                ) {
            ...
            const t = setInterval(() => {
                if (document.readyState === "complete") {
                    if (this.options.pane === "container-round") {
                        ...
                        this.options.paneContainer.setAttribute(
                            "style",
                            customStyle
                        );
                    }
