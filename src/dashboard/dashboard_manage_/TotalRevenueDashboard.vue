           ...
           <div class="d-flex justify-content-between align-itmes-center">
                <div>
                    <div class="p-3 rounded bg-soft-info">
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" shape-rendering="geometricPrecision"
                            text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd"
                            clip-rule="evenodd" viewBox="0 0 512 511.995">
                ...
                <!-- Select Date -->
                <div>
                    <div :class="{'dropdown' : defaultSelectedDateRevenue}">
                            <!-- Toggle Name -->
                            <a href="#" class="text-secondary dropdown-toggle" 
                                id="dropdownMenuButton1" 
                                data-bs-toggle="dropdown" aria-expanded="false">
                                <template v-if="selectedIdRevenue === 1 ">
                                    This Current Date
                                </template>
                                <template v-if="selectedIdRevenue === 2 ">
                                    This Week
                                </template>
                                <template v-if="selectedIdRevenue === 3 ">
                                    This Month
                                </template>
                                <template v-if="selectedIdRevenue === 4 ">
                                    This Year
                                </template>
                                <template else>
                                    This Current Date
                        ...
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton1">
                            <li v-for="item in revenuedItem" :key="item?.id" @click="onChangeFilterOrderByDateRevenue(item?.id)" >
...
export default {
    ...
    methods: {
        ...
        onChangeFilterOrderByDateRevenue(id){
            ...
            // Get Order
            this.productMSDashboardService.getDashboardTotalRevenue(id ?? 1).then((result) => {
                // console.log(result.totalRevenueNow[0].orderCount)
                if(!result){
                    this.orderCountRevenue = 0;
                    this.orderProductsTotalRevenue = 0;
                }
                if(result.totalRevenueNow && result.totalRevenueNow !== undefined){
                    this.orderCountRevenue = parseInt(result.totalRevenueNow[0].orderProductsTotal) ?? 0;
                    this.orderProductsTotalRevenue = result.totalRevenueNow[0].orderCount ?? 0;
                }
            }).catch((err) => {
