                            ...
                            <div class="grid formgrid">
                                ...
                                <div class="col-6 field">
                                    <div class="field">
                                        ...
                                        <small v-if="(v$.projectNameBaseRole.$invalid && submitted) || v$.projectNameBaseRole.$pending.$response" class="p-error text-sm">
                                ...
                                <div class="col-6">
                                    <div class="field">
                                        <label
                                            ...
                                            :class="{ 'p-invalid border-round-lg border-round-lg p-error': v$.modelCountryNameSelected.$invalid && submitted }"
                                        >
                                        ...
                                        <div class="flex field flex-row">
                                            <Dropdown 
                                                showClear
                                                v-model="v$.modelCountryNameSelected.$model"
                                                :class="{ 'p-invalid border-round-lg p-error': v$.modelCountryNameSelected.$invalid && submitted }"
                                                :options="getOptCountryOnProject" 
                                            ...
                                            <popup-add-manage-department-position-geo
                                                ...
                                                :projectId="getProjectById ? getProjectById : 0"
                                                :geoFenceLocation="getCountryId ? getCountryId : 0"
                                            />
                                        </div>
                                        <!-- Validations -->
                                        <small v-if="(v$.modelCountryNameSelected.$invalid && submitted) || v$.modelCountryNameSelected.$pending?.$response" class="p-error text-sm">
                                            {{ v$.modelCountryNameSelected.required.$message.replace('Value', 'Country') }}
                                        </small>
                                    </div>
                                </div>
...
* */ 
import PopupAddManageDepartmentPositionGeo from "../manage_position_department_geo/department_geo_by_country/AddManageDepartmentPositionGeoCountry.vue";
...
export default {
    ...
    created() {
        ...
        this.geoLocationServicesOnProject = new GeoLocationsManagementServices();
    },
    computed:{
        getProjectById() {
           return parseInt(this.projectNameBaseRole?.id) 
                ? parseInt(this.projectNameBaseRole?.id) : 0;
        },
        getCountryId(){
           return parseInt(this.modelCountryNameSelected?.id) 
                ? parseInt(this.modelCountryNameSelected?.id) : 0;
        }
    },  
    data() {
        return {
        ...
        * */ 
        getGeoLocationCountry(){
            try{
                ...
                this.geoLocationServicesOnProject.listGeoLocationCountryByZip(countryZipTypeCountry, superSSNCountryCode).then((country) => {
                    if (!country) {
                        this.getOptCountryOnProject = [];
                    }
                    this.getOptCountryOnProject = Array.isArray(country) ? country.slice() : [];
                }).catch((error) => {
