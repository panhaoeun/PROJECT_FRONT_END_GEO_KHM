                    ...
                    <div>
                        <div class="px-2">
                            ...
                            <DataTable
                                ...
                                v-model:selection="selectedPermModule"
                                dataKey="id"
                                :paginator="true"
                                :rows="10"
                                :filters="filters"
                            ...
                            >
                                ...
                                <div
                                    v-if="
                                        ...
                                        permissionsRoleModuleListArr.length >
                                            0 &&
                                        permissionsRoleModuleListArr != ''
                                    "
                                >
                        ...
                        <Dialog
                            ...
                            :header="
                                'Edit Permissions -   ' + currentRole?.role_name
                            "
                        >
                            <div
                                    ...
                                    <div class="block">
                                        ...
                                        >
                                            <el-table
                                                ...
                                                :data="
                                                    moduleDetailRoutes.filter(
                                                        (data) =>
                                                            !search ||
                                                            data.moduleParentName
                                                                .toLowerCase()
                                                                .includes(
                                                                    search.toLowerCase()
                                                                )
                                            ...
                                            >
                                                ...
                                                <el-table-column
                                                    ...
                                                    type="index"
                                                    :index="
                                                        indexMethodModulePerm
                                                    "
                                                >
                                                </el-table-column>
                                                <el-table-column
                                                    prop="moduleParentName"
                                                    label="Module Name"
                                                    sortable
                                                    width="260"
                                                >
...
export default {
    ...
    computed: {
        currentRole() {
            const permissionModule = this.permissionsRoleModuleListArr.find(
                (module) => module?.id === this.currentRoleId
            );
    ...
    methods: {
        ...
        tableRowClassNamePerm({ row }) {
            return `permId-${row?.permissionId ?? 0} roleId-${
                row?.roleId ?? 0
            } rolePermId-${row?.id}`;
        },
        /**
        ...
        handleEditPermissionsModules(id) {
            ...
            this.$nextTick(() => {
                this.userPermModuleMSServices
                    .getDetailModuleRoleFunctionByRoleId(this.currentRoleId)
                    .then((moduleId) => {
                        if (!moduleId) {
                            ElMessage.error("Internal Error...");
                        }
                        this.routesModule = this.generateRoutes(
                            moduleId?.moduleDetail
                        );
                        this.rolesPermModule = deepClone(
                            moduleId?.moduleDetail
                        );
        ...
        generateRoutes(moduleArr) {
            ...
            for (let modules of moduleArr) {
                const onlyOneShowingChid = this.onlyOneShowingChild(
                    modules.childrenModule,
                    modules
                );
        ...
        normalizeMenuPermission(permission) {
            return {
                id: permission.id,
                moduleParentName: permission.moduleParentName,
            };
        },
        ...
        changeSavePermissionModules(
            ...
            this.$nextTick(() => {
                try {
                    ...
                    const tdElTagChidTrModulePerm =
                        tblClosetTrModule.getElementsByTagName("td");
                    if (tdElTagChidTrModulePerm !== undefined) {
                        // Child Module
                        const childModulePermView =
                            tdElTagChidTrModulePerm[2].childNodes[0];
                        const childModulePermEdit =
                            tdElTagChidTrModulePerm[3].childNodes[0];
                        const childModulePermCreate =
                            tdElTagChidTrModulePerm[4].childNodes[0];
                        const childModulePermRemove =
                            tdElTagChidTrModulePerm[5].childNodes[0];
                        // Child Modules
                        const childModuleViewId =
                            childModulePermView.children[0].value ?? 0;
                        const childModuleEditId =
                            childModulePermEdit.children[0].value ?? 0;
                        const childModuleCreateId =
                            childModulePermCreate.children[0].value ?? 0;
                        const childModuleRemoveId =
                            childModulePermRemove.children[0].value ?? 0;
                        // Data Permissions Modules
                        const modulePermDetail = {
                            rolePermId: parseInt(rolePermId),
                            roleId: parseInt(roleIdModule)
                                ? parseInt(this.currentRoleId)
                                : 1,
