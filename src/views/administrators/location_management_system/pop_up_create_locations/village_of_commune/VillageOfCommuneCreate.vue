<template>
    <div class="col-12 field">
        ...
        >
            ...
            <ListVillageByCommune :geoLocalVillage="villageCommuneId ? villageCommuneId : ''" />
        </div>
        <!-- Form -->
        <div>
            <!-- Submitted -->
            ...
            >
                <div
                    class="dply-felx flex justify-content-between mtb-20 mtb-sm-15 oflow-hidden"
                >
                    <h6 class="font-bold text-sm mb-3 plr-20 mtb-5">
                        Village of commune
                        <span class="font-bold text-red-500">{{
                            nameGeoVillage ? nameGeoVillage : ""
                        }}</span>
                        - {{ index + 1 }}
                    </h6>
                    <!-- Button Actions -->
                ...
                <div class="pop-over-content p-20 p-sm-15 card">
                    <div class="flex gap-15">
                        ...
                        <div class="input-wrap flex-1">
                            <label
                                :class="{
                                    'p-invalid border-round-lg border-round-lg p-error':
                                        v.moreVillageCommune.$each.$response
                                            .$errors[index].stateCodeVillage
                                            .length && submitted,
                                }"
                            >
                                Zip Code
                            ...
                            <small
                                v-if="
                                    v.moreVillageCommune.$each.$response.$data[
                                        index
                                    ].stateCodeVillage.$invalid && submitted
                                "
                                class="p-error text-sm"
                            >
                                {{
                                    v.moreVillageCommune.$each.$response.$errors[
                                        index
                                    ].stateCodeVillage[0].$message.replace(
                                        "Value",
                                        "Zip Code"
                                    )
                                }}
                        ...
                        <div class="input-wrap flex-1">
                            ...
                            >
                                {{
                                    v.moreVillageCommune.$each.$response.$errors[
                                        index
                                    ].stateKhmerNameVillage[0].$message.replace(
                                        "Value",
                                        "Khmer Name"
                                    )
                                }}
                        ...
                        <div class="input-wrap flex-1">
                            <label
                                :class="{
                                    'p-invalid border-round-lg border-round-lg p-error':
                                        v.moreVillageCommune.$each.$response
                                            .$errors[index]
                                            .stateEnglishNameVillage.length &&
                                        submitted,
                                }"
                            >
                                English Name
                            ...
                            <small
                                v-if="
                                    v.moreVillageCommune.$each.$response.$data[
                                        index
                                    ].stateEnglishNameVillage.$invalid &&
                                    submitted
                                "
                                class="p-error text-sm"
                            >
                                {{
                                    v.moreVillageCommune.$each.$response.$errors[
                                        index
                                    ].stateEnglishNameVillage[0].$message.replace(
                                        "Value",
                                        "English Name"
                                    )
                                }}
                        ...
                        <div class="input-wrap flex-1">
                            ...
                            >
                                {{
                                    v.moreVillageCommune.$each.$response.$errors[
                                        index
                                    ].stateLongitudeVillage[0].$message.replace(
                                        "Value",
                                        "Longitude"
                                    )
                                }}
                        ...
                        <div class="input-wrap flex-1">
                            <label
                                :class="{
                                    'p-invalid border-round-lg border-round-lg p-error':
                                        v.moreVillageCommune.$each.$response
                                            .$errors[index].stateLatitudeVillage
                                            .length && submitted,
                                }"
                            >
                                Latitude
                            ...
                            <small
                                v-if="
                                    v.moreVillageCommune.$each.$response.$data[
                                        index
                                    ].stateLatitudeVillage.$invalid && submitted
                                "
                                class="p-error text-sm"
                            >
                                {{
                                    v.moreVillageCommune.$each.$response.$errors[
                                        index
                                    ].stateLatitudeVillage[0].$message.replace(
                                        "Value",
                                        "Latitude"
                                    )
                                }}
            ...
            <div class="gap-15">
                <div class="col-12 flex justify-content-end mt-4">
                    ...
                    <Button
                        icon="pi pi-check"
                        class="p-button-lg btn btn-primary w-13rem h-4rem"
                        type="submit"
...
export default {
    ...
    computed: {
        nameGeoVillage() {
            return this.geoVillageName?.geo_english_name || "";
        },
        geoVillageCommuneSSNLocation() {
            return (
                this.geoVillageName?.geo_ssn_location &&
                this.geoVillageName?.geo_ssn_location
            );
        },
        getSelectedCommuneId() {
            if (
                !Array.isArray(this.villageCommuneId) ||
                this.villageCommuneId !== undefined ||
                this.villageCommuneId !== null
            ) {
                const getCommuneOptSelected = this.villageCommuneId
                    ? this.villageCommuneId
                    : "";
    ...
    methods: {
        ...
        async submitFormVillageCommune() {
            this.submitted = true;
            this.loadingVillageSubmitted = true;
            this.v.$touch();
            // Check Selected Province
            if (!this.villageCommuneId || this.villageCommuneId === null) {
                this.$notify.error({
                    title: "Please selected commune or town",
                    message: "Selected commune or town is required",
                });
                throw new Error("Please selected commune or town is required");
            }
            // display form values on success
            let arrayCommuneVillageObj = [];
            const arrayCommuneVillage = this.state?.moreVillageCommune
            ...
            for (let index = 0; index < arrayCommuneVillage.length; index++) {
                ...
                obj.geoSuperSSNCommune = this.villageCommuneId
                    ? this.villageCommuneId
                    : null;
                obj.addNewGeoCountryVillageZipCode = String(
                    countryIndex?.stateCodeVillage
                );
                obj.addNewGeoCountryVillageKhmerName =
                    countryIndex?.stateKhmerNameVillage;
                obj.addNewGeoCountryVillageEnglishName =
                    countryIndex?.stateEnglishNameVillage;
                obj.addNewGeoCountryVillageLongitude =
                    countryIndex?.stateLongitudeVillage;
                obj.addNewGeoCountryVillageLatitude =
                    countryIndex?.stateLatitudeVillage;
            ...
            const villageCommuneAddNewDetail = {
                geoVillageCapitalDetail: arrayCommuneVillageObj
                    ? arrayCommuneVillageObj
                    : [],
            };
            setTimeout(() => {
                this.geoLocationServices
                    .createVillageGeoLocation(villageCommuneAddNewDetail)
                    .then(async (response) => {
                    ...
                    .catch((error) => {
                        ...
                        this.$notify.error({
                            ...
                            message:
                                ...
                                "Unsuccessfully create geo-location village by commune",
                            showClose: false,
                        });
                        if (error.response.data.error.error?.errors) {
                            for (
                                ...
                                this.$notify.error({
                                    title: "Unsuccessfully create geo-location village of commune",
                                    message:
                                        messageValidation ??
                                        "Unsuccessfully create geo-location village by commune",
                                    showClose: true,
            ...
            if (isFormCorrect !== true || isFormCorrect == false) {
                this.$toast.add({
                    ...
                    detail: "Please input filed commune or town have missing value!",
                    life: 3000,
                });
                return false;
            }
