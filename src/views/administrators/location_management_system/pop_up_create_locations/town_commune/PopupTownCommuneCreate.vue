...
export default {
    ...
    methods: {
        ...
        async submittedCommuneByDistrict() {
            this.submitted = true;
            this.v.$touch();
            ...
            if (
                !this.geoDistrictSSNCommuneId ||
                this.geoDistrictSSNCommuneId === null
            ) {
                this.$notify.error({
                    title: "Please selected district",
                    message: "Selected district is required",
                });
                throw new Error("Please selected district is required");
            }
            // Add News Commune
            ...
            for (let index = 0; index < arrayDistrictProvince.length; index++) {
                ...
                obj.geoSuperSSNDistrictCountry = this.geoDistrictSSNCommuneId
                    ? this.geoDistrictSSNCommuneId
                    : null;
                (obj.addNewGeoCountryCommuneZipCode = String(
                    countryIndex?.stateCode
                )),
                    (obj.addNewGeoCountryCommuneKhmerName =
                        countryIndex?.stateKhmerName),
                    (obj.addNewGeoCountryCommuneEnglishName =
                        countryIndex?.stateLatinName),
                    (obj.addNewGeoCountryCommuneLongitude =
                        countryIndex?.stateLongitude),
                    (obj.addNewGeoCountryCommuneLatitude =
            ...
            setTimeout(() => {
                const communeAddNewDetail = {
                    geoCommuneCapitalDetail: arrayProvinceCommuneObj
                        ? arrayProvinceCommuneObj
                        : [],
                };
                this.geoLocationServices
                    .createCommuneGeoLocation(communeAddNewDetail)
                    .then(async (response) => {
                    ...
                    .catch((error) => {
                        this.$notify.error({
                            ...
                            message:
                                ...
                                "Unsuccessfully create geo-location commune",
                            showClose: false,
                        });
                        this.submittedLoadingBtn = false;
