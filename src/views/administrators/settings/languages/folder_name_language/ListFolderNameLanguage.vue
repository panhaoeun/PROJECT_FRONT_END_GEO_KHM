<template>
    <!-- Button Geo Country -->
    <button class="ajax-btn outline-btn plr-20 mtb-5 border-round" icon="pi pi-plus" type="button" label="New"
        aria-label="New" @click.prevent="popUpCreateFolderLanguage()">
        <span>
            Edit
    ...
    <Dialog v-model:visible="openDialog" header="List of Folder" :style="{ width: '75vw' }" modal
        :contentStyle="{ height: '600px' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" :draggable="false"
        ...
        <div class="shipping-rule mb-20 mb-sm-15 border-1 border-primary-100 card border-round gap-15">
            <DataTable  class="p-datatable-scrollable text-sm"
                :rows="10"
                dataKey="id" 
                :paginator="true" 
                :value="getAllFolderLanguage" 
                :rowHover="true" 
                contextMenu 
                v-model:filters="filtersFolderLanguage" 
                filterDisplay="menu"
                :loading="loadingCountry" 
                :filters="filtersFolderLanguage" 
                responsiveLayout="scroll"
                :globalFilterFields="['folder_id', 'tool_description']"
            ...
            >
                ...
                <template #loading>
                    Loading Folder data. Please wait.
                </template>
                <Column selectionMode="multiple" :style="{ width: '3rem' }" :exportable="false"></Column>
                <Column field="folder_id" header="Folder Name" sortField="folder_id" sortable>
                    <template #body="{ data }">
                        {{ data?.folder_id }}
                    </template>
                    <!-- Folder Name Filter -->
                ...
                <Column field="tool_description" header="Note" sortField="tool_description" sortable>
                    <template #body="{ data }">
        ...
        <!-- Pop Edited Country -->
        <edit-folder-name-language v-if="openEditedCountryCountry" :geoLocalCountry="editCountryPopup"
...
import EditFolderNameLanguage from "./EditFolderNameLanguage.vue";
...
export default {
    ...
    computed: {
        ...mapGetters('folderlanguage', ['folderLanguageAll']),
        getAllFolderLanguage() {
        return this.folderLanguageAll || [];
    }
    },
    data() {
        return {
            filtersFolderLanguage: {
                global: { value: null, matchMode: FilterMatchMode.CONTAINS },
                folder_id: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
                tool_description : { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            },
            openDialog: false,
    ...
    methods: {
        ...
        popUpCreateFolderLanguage() {
            this.openDialog = true;
        },
        onRowEditSave(event) {
            let { newData, index } = event;
            this.getAllFolderLanguage[index] = newData;
        },
        onPage(event) {
