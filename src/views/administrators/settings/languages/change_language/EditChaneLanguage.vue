<template>
    <form @submit.prevent="submittedDialogEditGeoLocalCountry">
        ...
        <pop-over v-if="geoLocationCountryData" :title="`Edit Country:` + ' ' + nameCountry" @close="$emit('close')"
            elem-id="user-address-pop-over" :layer="true" class="address-popup popup-top-auto">
            <!-- Contents -->
            <template v-slot:content>
                ...
                <div class="flex start mlr--5">
                    <div class="input-wrap mlr-5 col-12 pl-0"
            ...
            <template v-slot:pop-footer>
                <div class="flex j-end gap-10">
                    <button class="outline-btn plr-30 plr-sm-15 border-round" aria-label="Country cancel"
                        @click.prevent="$emit('close')">
                        {{ $t('addressPopup.cancel') }}
                    </button>
                    <ajax-button class="primary-btn  plr-30 plr-sm-15" :fetching-data="submittingCountryData"
