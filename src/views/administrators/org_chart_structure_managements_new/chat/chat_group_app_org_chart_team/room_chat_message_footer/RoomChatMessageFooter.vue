 ...
 * Global Functions
 * */
import EmojiPickerContainer from "./emoji_pickers_container/EmojiPickerComponents";
    ...
    methods: {
        ...
        async onFileChange(files) {
            ...
            Array.from(files).forEach(async (file) => {
                ...
                let loadedFile = this.files.find(
                    (file) => file.localUrl === fileURL
                );
        ...
        updateFooterList(tagChar) {
            ...
            if (
                tagChar === "@" &&
                (!this.userTagsEnabled || !this.room.users)
            ) {
                return;
            }
