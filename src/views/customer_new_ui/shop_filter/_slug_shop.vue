    ...
    <div>
            ...
            <div class="container-fluid">
                <div class="store-info">
                    <div class="left-area">
                        ...
                        <!-- Store Contents -->
                        <div
                            class="store-content flex align-items-center justify-content-center"
                        >
                            <div>
                                <h4 class="bold" style="margin-bottom: 0px">
                                    {{ store?.shop_eng }}
                                </h4>
                            </div>
                            <div>
                                <p class="opacity-8 f-9">Member since</p>
                                <h6>
                                    <b>{{ storeDate }}</b>
                                </h6>
                            </div>
        ...
        <div class="container-fluid mb-50 mb-sm-20">
            <div class="flex sided mb-15">
                <breadcrumb class="mtb-0" :page="store?.shop_eng" />
                <!-- <sort-by
...
export default {
    ...
    computed: {
        ...
        storeDate() {
            return moment(this.filtersProductByShopBrand?.createdAt).format(
                "MMM DD, YYYY"
            );
        },
        currentItems() {
    ...
    methods: {
        ...
        getCommonCategoriesFIlter() {
            this.commonServices
                .getCommonCategoriesSubCategories()
                .then((common) => {
                    if (!common) {
                        this.commonCategoriesFilterList = Array.isArray() ?? [];
                        this.commonSubCategoriesFilterList =
                            Array.isArray() ?? [];
                    }
        ...
        async getSellerProductTypeByShopFilter() {
            const shopTypeIdFilter = this.$route.params.shopId
                ? this.$route.params.shopId
                : 0;
            // Filter Id
            this.commonPublic
                .searchProductByShopCategoriesSeller(shopTypeIdFilter)
                .then((sellerProduct) => {
                    if (
                        !Array.isArray(sellerProduct) ||
                            ...
                            if (
                                ...
                                this.filtersProductByShopBrand =
                                    sellerProduct.shop
                                        ? sellerProduct.shop
                                        : "";
                                this.shopLocationSeller =
                                    sellerProduct.locationShop
                                        ? sellerProduct.locationShop
                                        : "";
                                this.loadingProductList = false;
                            }
                        }, 100);
                        self.products = this.currentItems
                            ? this.currentItems
                            : [];
                        self.fetchingProductData = false;
                    } catch (e) {
