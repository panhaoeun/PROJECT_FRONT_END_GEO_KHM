                    ...
                    <div>
                        <div class="px-2">
                            ...
                            <DataTable ref="dt" :value="catList" v-model:selection="selectedCategoriesList" dataKey="id"
                                ...
                                :rowsPerPageOptions="[5, 10, 25]" :metaKeySelection="false"
                                ...
                                <div v-if="catList && catList.length > 0 && catList != ''">
                                    <!-- Columns -->
                                    <Column field="id" :header="$t('category.categoryImage')" sortable
                                        style="min-width:15rem">
                                        <template #body="{ data }">
                                            <div class="flex flex-column">
                                                <div class="flex px-2 py-2">
                                                    <router-link
                                                        :to="{ path: '/vendor/products/sub-category/list', id: data.catID }"
                                                        class="flex px-1 py-1">
                                                        <Avatar :image="`${envVueHost}uploads/${data.catLogo}`" class="mr-2"
                                                            shape="circle" :data-id="data" />
                                                        {{ data.catNameEn }}
                                                    </router-link>
                                                </div>
                                            </div>
