                    ...
                    <el-card class="box-card h-100" v-if="customerDetailOrder.length > 0 && customerDetailOrder !== ''">
                        <div class="flex justify-content-between">
                            <div class="relative text-black text-xl section section-title:before">
                                <p class="h6">Order ID #{{ customerDetailOrder[0]?.invoiceNoId ?
                                    customerDetailOrder[0]?.invoiceNoId : '' }}</p>
                                <span class="text-sm">{{ customerDetailOrder[0]?.orderDate ?
                                    customerDetailOrder[0]?.orderDate : '' }}</span>
                            </div>
                        </div>
                            ...
                            <div class="justify-content-center flex">
                                <div class="flex flex-column align-items-center justify-content-center mr-2">
                                    <div class="text-sm pb-3">
                                        ...
                                        <Tag severity="danger" :value="customerDetailOrder[0]?.order_status"></Tag>
                                    </div>
                                    <div class="text-sm">
                            ...
                            <div class="px-2 my-2">
                                ...
                                <div v-if="orderItems && orderItems.length > 0 && orderItems !== ''">
                                    <el-table :data="orderItems" class="text-sm" stripe style="width: 100%">
                                        ...
                                        <el-table-column prop="variantName" label="Variations">
                                            <template #default="{ row }">
                                                <template v-if="row?.variantName && row?.variantName !== ''">
                                                    <template
                                                        v-for="([key, value], index) in Object.entries(row?.variantName)"
                                                        :key="index">
                                                        <div class="flex flex-column">
                                    ...
                                    <div class="flex flex-column py-3 my-2">
                                        <div class="border-round">
                                            ...
                                            <span class="font-bold">
                                                {{ currencyFormattedKHRiel(customerDetailOrder[0]?.shippingPrice) }}
                                            </span>
                                        </div>
                                        <div class="border-round">
                ...
                <div class="col-lg-4 grid col-xl-3 d-flex flex-column gap-3">
                    <div class="gap-3 column-gap-4 row-gap-6">
                                    ...
                                    <div class="col-12" style="width: 20rem;">
                                        <div class="field">
                                            ...
                                            <select class="form-select form-select-md w-full select-lg"
                                                ...
                                                <!-- Pending -->
                                                <option value="Pending" :selected="customerDetailOrder[0]?.order_status === 'Pending'">
                                                    Pending
                                                </option>
                                                <!-- Access Roles -->
                                                <template v-if="getCurrentRoleAccess !== '' && getCurrentRoleAccess !== undefined">
                                                    <!-- Manager -->
                                                    <template
                                                        v-if="getCurrentRoleAccess === 'Owner' || getCurrentRoleAccess === 'Manager'">
                                                        <option value="Processing"
                                                            :selected="customerDetailOrder[0]?.order_status === 'Processing'">
                                                            Processing</option>
                                                        <option value="Declined"
                                                            :selected="customerDetailOrder[0]?.order_status === 'Declined'">
                                                            Declined
                                                        </option>
                                                    </template>
                                                    
                                                    <!-- Seller -->
                                                    <template v-if="getCurrentRoleAccess === 'Seller'">
                                                        <!-- Check  -->
                                                        <template v-if="customerDetailOrder[0]?.order_status === 'Declined'">
                                                            <option disabled>{{ customerDetailOrder[0]?.order_status }}</option>
                                                        </template>
                                                        <template v-else>
                                                            <option value="Packing"
                                                                :selected="customerDetailOrder[0]?.order_status === 'Packing'">
                                                                Packing
                                                            </option>
                                                            <option value="Delivery"
                                                                :selected="customerDetailOrder[0]?.order_status === 'Delivery'">
                                                                Delivery
                                                            </option>
                                                        </template>
                                                    </template>
                        ...
                        <el-card class="box-card"
                            v-if="customerDetailOrder[0]?.shippingAddress !== '' && customerDetailOrder[0]?.shippingAddress !== null">
                            <div class="flex justify-content-between flex flex-wrap gap-3">
                                <div class="relative text-black text-sm section section-title:before">
                        ...
                        <!-- ===============Billing address ===========-->
                        <el-card class="box-card"
                            v-if="customerDetailOrder[0]?.billingAddress !== '' && customerDetailOrder[0]?.billingAddress !== null">
                            <div class="flex justify-content-between flex flex-wrap gap-3">
                                <div class="relative text-black text-sm section section-title:before">
                        ...
                        <el-card class="box-card" v-if="shopInfoItemOrder.length > 0 && shopInfoItemOrder !== ''">
                            ...
                            <div class="py-2">
                                <div class="mb-4 d-flex align-items-center gap-2">
                                    ...
                                    <img class="justify" v-if="shopInfoItemOrder[0]?.shopLogo"
                                        :title="shopInfoItemOrder[0]?.shopEn ?? ''"
                                    ...
                                    <div class="flex flex-column title-customer-information">
                                        ...
                                        <div class="pb-2"
                                            v-if="shopInfoItemOrder[0]?.billingAddress !== '' && shopInfoItemOrder[0]?.buildingAddress !== null">
                                            <strong>Building Address: </strong>
                                            <span>{{ shopInfoItemOrder[0]?.buildingAddress ?? '' }}</span>
                                        </div>
                                        <div class="text-sm pb-2"
                                            v-if="shopInfoItemOrder[0]?.shopAddr01 !== '' && shopInfoItemOrder[0]?.shopAddr01 !== null">
                                            <strong>City: </strong>
...
export default {
    ...
    methods: {
        ...
        changeStatusOrderConfirmByVendor(event) {
            ...
            this.$confirm('You wont be able to revert this!', `Are you sure Change this`, {
                showCancelButton: true,
                confirmButtonText: 'OK',
                cancelButtonText: 'Cancel',
                type: 'warning',
                cancelButtonClass: 'surface-hover font-bold hover:surface-300 w-7rem',
                confirmButtonClass: 'bg-red-500 border-none font-bold hover:surface-300 w-10rem',
                ...
                const orderStatus = event.target.value ? event.target.value : '';
                // Confirm Order
                const orderPayStatus = {
                    orderStatusConfirm: orderStatus ? orderStatus : 'Processing',
                    paymentStatus: ''
                }
                const orderParamId = parseInt(this.$route.params.orderId) ?? 0;
                this.cusOrderMSServices.updatedConfirmToPaymentsOrderByVendor(orderParamId, orderPayStatus)
                    .then((orderItem) => {
                        if (orderItem.data.success === true) {
                            this.$notify.success({
                                title: 'Successfully change order status',
                                message: orderItem.data?.message ? orderItem.data?.message : '',
                                showClose: false
                            });
                            //Close form -> Successfully to submitted   
                            this.centerDialogVisibleWithdrawAdmin = false;
                            this.$router.push('/vendor/order_managements/order_list');
                        } else {
                            this.$notify.error({
                    ...
                    .catch((error) => {
                        ...
                        if (error) {
                            this.$notify.error({
                                ...
                                message: error.response.data.error.message ?? 'Unsuccessfully for change status order item',
                                showClose: false
                            });
                            if (error.response.data.error.error.errors) {
                                for (let index = 0; index < error.response.data.error.error.errors.length; index++) {
                                    ...
                                    this.$notify.error({
                                        title: 'Unsuccessfully change order status...',
                                        message: messageValidation ?? 'Unsuccessfully for change status order item',
                                        showClose: true
                                    });
                                }
                            }
